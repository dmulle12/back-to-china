...
3. **Rule Sets**
   - `BackToChina.conf` already loads the hosted domain sets from this repository for ad blocking, Malaysian local services, Apple services, overseas/AI services, Chinese domains, and Chinese streaming services.
   - Keep the profile URL accessible so Surge can refresh these domain sets. To use a fork, replace `dmulle12/back-to-china` in the `DOMAIN-SET` URLs with your own repository path.
